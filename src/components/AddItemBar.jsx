...
export default function AddItemBar({ onAdd }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // 基本的 API 返回校验
      if (!data) {
        setErr('No data returned from API.');
        return;
      }
      if (data.error) {
        setErr(typeof data.error === 'string' ? data.error : 'API returned an error.');
        return;
      }

      // 当类别未知且未提供 days 时，提示用户输入
      ...
      // 更明确的错误信息
      const message = e2?.message ? `API error: ${e2.message}` : 'Failed to add item (network or unexpected error).';
      setErr(message);
  ...
  return (
    <form onSubmit={handleSubmit}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
        ...
        {loading ? (
          <div aria-live="polite" style={{ color:'#6b7280', fontSize:13 }}>Loading…</div>
        ) : null}
      </div>

      {err ? (
        <span role="alert" aria-live="assertive" style={{ color:'#c00' }}>
          Error: {err}
        </span>
      ) : null}
    </form>
