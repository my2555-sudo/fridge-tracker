...
function includesAny(s, arr) {
  s = norm(s);
  return arr.some(k => s.includes(norm(k)));
}
...
export async function fetchOffByBarcode(barcode, opts = {}) {
  ...
  try {
    ...
    throw new Error('Failed to fetch product by barcode. Please check your network or try again later.');
  }
}

/** ---------- 名称查询（多取→择优） ---------- */
export async function fetchOffByName(query, opts = {}) {
  if (!query) throw new Error('Missing name');
  const url =
  ...
  try {
    ...
    if (!list.length) return { error: 'No matching product found.' };
    ...
    throw new Error('Failed to fetch product by name. Please check your network or try again later.');
  }
}
